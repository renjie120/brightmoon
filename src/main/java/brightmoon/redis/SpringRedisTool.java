 ...
 * 
 */
public class SpringRedisTool implements IRedisTool {
	...
	public String getJsonData(final byte[] key) {
		return template.execute(new RedisCallback<String>() {
			...
			public String doInRedis(RedisConnection connection)
				...
				DataType tp = connection.type(key);
				if (DataType.STRING.equals(tp)) {
					return "{'type':'string','value':'"+new String(connection.get(key))+"'}";
				} else if (DataType.HASH.equals(tp)) {
					Map<byte[],byte[]> m =  connection.hGetAll(key); 
					Iterator<Entry<byte[],byte[]>> it = m.entrySet().iterator();
					...
					return "{'type':'hash','value':"+bui.append("]").toString()+"}";
				} else if (DataType.LIST.equals(tp)) {
					...
					return "{'type':'list','value':"+bui.append("]").toString()+"}";
				} else if (DataType.SET.equals(tp)) {
					...
					return "{'type':'set','value':"+bui.append("]").toString()+"}";
				} else if (DataType.ZSET.equals(tp)) {
					Set<Tuple> s=  connection.zRangeWithScores(key, 0, -1); 
					StringBuilder bui = new StringBuilder(100);
					bui.append("[");
					for(Tuple b:s){
						bui.append("{'value':'"+new String(b.getValue())+"','score':'"+b.getScore()+"'},");
					}
					if(s!=null&&s.size()>0){
						bui = bui.deleteCharAt(bui.lastIndexOf(","));
					}
					return "{'type':'zset','value':"+bui.append("]").toString()+"}";
				} else
					return "";
			}
