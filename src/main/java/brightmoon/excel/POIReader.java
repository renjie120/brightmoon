 ...
 * 
 */
public class POIReader extends ExcelTool implements IExcelReader {
	/**
	 * 工作簿
	 */
	...
	public String getSheetName(int num) {
		setCurrentSheet(num);
		String result = sheet.getSheetName();
		return result;
	}
	 ...
	 * @return
	 */
	private HSSFRow getRow(int sheetId, int rowId) {
	...
	public String getSheetNum() {
		try {
			String result = new Integer(workbook.getNumberOfSheets())
					.toString();
			return result;
		} catch (Exception e) {
			error(e);
